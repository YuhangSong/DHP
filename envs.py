...
class PanoramicEnv():
    def __init__(self, env_id, task, subject=None, summary_writer=None):
        ...
        class nnn():
            def __init__(self, n):
                self.n = n
        self.action_space = gym.spaces.Discrete(config.direction_num)
    ...
    def step(self, action, v):
        ...
        if done:
            ...
            to_log["global/cur_cc"] = self.cc_cur_video
            to_log["global/max_cc"] = self.max_cc_cur_video
        ...
        if config.mode is 'off_line':
            return self.cur_observation, reward, done, to_log, v_lable
        elif config.mode is 'on_line':
            return self.cur_observation, reward, done, to_log, v_lable, self.predicting
