...
def get_view(input_width,input_height,view_fov_x,view_fov_y,view_center_lat,view_center_lon,output_width,output_height,cur_frame,file_,is_render=False,temp_dir=""):
    ...
    if(is_render==True):
        ...
        frame = np.reshape(frame, [output_width, output_height, 1])
...
def get_pic(input_width,input_height,view_fov_x,view_fov_y,view_center_lat,view_center_lon,output_width,output_height,cur_frame,file_,is_render=False,temp_dir=""):
    temp_1=temp_dir+"1.yuv"
    # print(temp_1)
    # print(stop)
    import config
    ...
    if(is_render==True):
        ...
        frame = frame.astype(np.float32)
        frame *= (1.0 / 255.0)
        frame = np.reshape(frame, [output_width, output_height])
        print(frame.shape)
        print(input_width)
